...
<script>
	...
	// styling
	let className = 'wickselect-control';
	...
	itemConfig.optionProps =
		value && valueAsObject && (multiple && Array.isArray(value) ? value.length > 0 : true)
	...
	let initFetchOnly =
		fetchMode === 'init' ||
		(fetchMode === 'auto' && typeof fetch === 'string' && fetch.indexOf('[query]') === -1);
			...
			fetchSource(query, fetchCallback)
				.then((data) => {
					if (!Array.isArray(data)) {
						console.warn('[wickselect]:Fetch - array expected, invalid property provided:', data);
						data = [];
					}
					options = data;
				})
	...
	/** - - - - - - - - - - STORE - - - - - - - - - - - - - -*/
	let selectedOptions =
		value !== null ? initSelection.call(options, value, valueAsObject, itemConfig) : [];
	...
	function handleValueUpdate(passedVal) {
		...
		if (passedVal) {
			...
			let success =
				...
				(multiple ? passedVal.length === _selection.length : _selection.length > 0);
			if (!success) {
				// this is run only when invalid 'value' is provided, like out of option array
				console.warn('[wickselect]: provided "value" property is invalid', passedVal);
				value = null;
				readSelection = null;
				return;
			}
	...
	function onSelect(event, opt) {
		...
		if (!multiple) {
			...
			tick().then(() => {
				dropdownActiveIndex = maxReached ? null : listIndex.next(dropdownActiveIndex - 1, true);
			});
		}
		emitChangeEvent();
	}
		...
		switch (event.key) {
			...
			case 'Enter':
				...
				if (creatable && $inputValue) {
					activeDropdownItem = !activeDropdownItem || ctrlKey ? $inputValue : activeDropdownItem;
					ctrlKey = false;
				}
...
<div class={`wickselect ${className}`} class:is-disabled={disabled} {style}>
	<Control
		...
		{inputId}
		{resetOnBlur}
		collapseSelection={collapseSelection ? config.collapseSelectionFn : null}
		{inputValue}
		{hasFocus}
		{hasDropdownOpened}
		{selectedOptions}
	...
	{#if name && !hasAnchor}
		...
		>
			{#each selectedOptions as opt}
				<option value={opt[labelAsValue ? currentLabelField : currentValueField]} selected
					>{opt[currentLabelField]}</option
				>
			{/each}
		</select>
	{/if}
...
<style>
	...
	.wickselect {
		@apply relative flex-auto text-base-content;
	}
	.wickselect.is-disabled {
		@apply cursor-none;
	}
	.icon-slot {
		display: flex;
	...
	:global(.wickselect-control .has-multiSelection .sv-item) {
		@apply bg-base-300 text-base-content mt-1 mr-1 mb-1 ml-0 rounded-md overflow-hidden;
	}
	:global(.wickselect-control .has-multiSelection .sv-item-content),
	:global(.wickselect-control .sv-dropdown-content .sv-item) {
		padding: 3px 3px 3px 6px;
	}
	:global(.wickselect-control .sv-item) {
		@apply flex justify-center items-center min-w-0 box-border rounded-md mb-1 cursor-pointer;
	}
	:global(.wickselect-control .sv-item.is-disabled) {
		opacity: 0.5;
		cursor: not-allowed;
	}

	:global(.wickselect-control .sv-item-content) {
		@apply text-ellipsis whitespace-nowrap box-border rounded-sm overflow-hidden w-full;
	}
	:global(.wickselect-control .sv-dd-item-active > .sv-item) {
		@apply bg-neutral text-neutral-content transition-colors duration-300;
	}
	:global(.wickselect-control .sv-dd-item-active > .sv-item .sv-item-content) {
		@apply text-neutral-content transition-colors duration-300;
	}
	:global(.wickselect-control .highlight) {
